...
for (int fi : fit_types.keys)
{
	...
	for (int pari : parameters.keys)
	{
		...
		for (int ei : errors.keys)
		{
			...
			RootObject g_data_unc = RootGetObject(f, parameters[pari] + "_unc/g_data");
			...
			g_data_unc.vExec("GetPoint", 0, ax, ay); real unc_mean = ax[0], unc_mean_unc = ay[0];

			real dy = 0.1;

			real y = ei - 0.02;
			draw((rms, y-dy)--(rms, y+dy), red);
			draw((rms - rms_unc, y)--(rms + rms_unc, y), red);

			real y = ei + 0.02;
			draw((unc_mean, y-dy)--(unc_mean, y+dy), blue);
			draw((unc_mean - unc_mean_unc, y)--(unc_mean + unc_mean_unc, y), blue);
		}
	}
}
